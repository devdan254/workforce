<x-student-layout title="Documents">
                                ...
                                <td class="text-end">
                                    <div class="d-flex justify-content-end align-items-center gap-2">
                                        @if(in_array($document->status, ['under_review', 'verified']))
                                            <a href="{{ route('student.documents.download', $document) }}" class="btn btn-sm btn-outline-secondary">View</a>
                                        @endif
                                        @if(in_array($document->status, ['required', 'rejected']))
                                            <form method="POST" action="{{ route('student.documents.upload', $document) }}" enctype="multipart/form-data" class="d-flex gap-1">
                                                @csrf
    ...
    @empty
        <div class="card stat-card p-5 text-center text-secondary">
            No documents required yet — these appear once you start an application, or you can add one below.
        </div>
    @endforelse
                ...
                <form method="POST" action="{{ route('student.documents.store') }}" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="form-text">If you attach more than one file below, they'll be numbered automatically — e.g. "Bank Statement (1)", "Bank Statement (2)".</div>
                        </div>
