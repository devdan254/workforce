{{--
    Mirrors admin/job-seekers/partials/_document-actions.blade.php exactly —
    same reasoning, just employers.* route names since admin.employers.* is
    its own separate route group.
    Expects: $document, $employer, $categories, $prefix (unique per context).
--}}
<div class="d-flex gap-1 flex-wrap justify-content-end">
    ...
    @if($document->status === 'under_review')
        <form method="POST" action="{{ route('admin.employers.documents.verify', [$employer, $document]) }}">
            @csrf
    ...
    @can('delete', $document)
        <form method="POST" action="{{ route('admin.employers.documents.destroy', [$employer, $document]) }}" onsubmit="return confirm('Delete this document? This cannot be undone.');">
            @csrf
            @method('DELETE')
...
<div class="modal fade" id="upload-{{ $prefix }}-{{ $document->id }}" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <form method="POST" action="{{ route('admin.employers.documents.upload', [$employer, $document]) }}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="modal-body">
                    <p class="text-secondary small">Use this when you've received the file directly (email, in person) and are logging it on the employer's behalf.</p>
...
<div class="modal fade" id="reject-{{ $prefix }}-{{ $document->id }}" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <form method="POST" action="{{ route('admin.employers.documents.reject', [$employer, $document]) }}">
                @csrf
                <div class="modal-header">
                ...
                <div class="modal-body">
                    <textarea name="reason" class="form-control" rows="3" required placeholder="Reason the employer will see"></textarea>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-light" data-bs-dismiss="modal">Cancel</button>
...
<div class="modal fade" id="edit-{{ $prefix }}-{{ $document->id }}" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <form method="POST" action="{{ route('admin.employers.documents.update', [$employer, $document]) }}">
                @csrf
                @method('PATCH')
                <div class="modal-header">
