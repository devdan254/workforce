<x-employer-layout title="Ticket Details">

    @if(session('success'))
        <div class="alert alert-success">{{ session('success') }}</div>
    @endif

    <a href="{{ route('employer.support.index') }}" class="text-decoration-none small mb-3 d-inline-block">← Back to Support</a>
    ...
    @if($ticket->status !== 'closed')
        <div class="card stat-card p-4">
            <form method="POST" action="{{ route('employer.support.reply', $ticket) }}">
                @csrf
                <div class="mb-3">
