<x-job-seeker-layout title="Documents">
                                ...
                                <td class="text-end">
                                    <div class="d-flex justify-content-end align-items-center gap-2">
                                        @if(in_array($document->status, ['under_review', 'verified']))
                                            <a href="{{ route('job-seeker.documents.download', $document) }}" class="btn btn-sm btn-outline-secondary">View</a>
                                        @endif
                                        @if(in_array($document->status, ['required', 'rejected']))
                                            <form method="POST" action="{{ route('job-seeker.documents.upload', $document) }}" enctype="multipart/form-data" class="d-flex gap-1">
                                                @csrf
    ...
    @empty
        <div class="card stat-card p-5 text-center text-secondary">
            No documents yet — these appear once you apply for a job, or you can add one below.
        </div>
    @endforelse
    ...
    <div class="modal fade" id="addDocumentModal" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <form method="POST" action="{{ route('job-seeker.documents.store') }}" enctype="multipart/form-data">
                    @csrf
