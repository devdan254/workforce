<x-job-seeker-layout title="Visa Status">

    <a href="{{ route('job-seeker.applications.show', $application) }}" class="text-decoration-none small mb-3 d-inline-block">← Back to Application</a>

    <div class="card stat-card p-4 mb-4">
        <h2 class="h5 fw-semibold mb-1" style="font-family:'Poppins',sans-serif;color:#082159;">
            {{ $visa?->destination_country ?? $application->jobPosting->country }} Work Visa
        </h2>
        <p class="text-secondary mb-3">{{ $application->jobPosting->title }}</p>
    ...
    @else
        <div class="card stat-card p-4 text-center text-secondary">
            Your visa process begins once your job offer is accepted. Nothing to show yet.
        </div>
    @endif
