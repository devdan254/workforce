<x-job-seeker-layout title="Notifications">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        @if($notifications->contains(fn($n) => is_null($n->read_at)))
            <form method="POST" action="{{ route('job-seeker.notifications.read_all') }}">
                @csrf
    ...
    <div class="card stat-card">
        @forelse($notifications as $notification)
            <form method="POST" action="{{ route('job-seeker.notifications.read', $notification->id) }}"
